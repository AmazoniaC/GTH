/**
 * Festivos de Colombia y conteo de días (hábiles/calendario) en el cliente.
 * Espejo de la utilidad del backend, para previsualizar la duración de una
 * ausencia antes de guardar. El valor definitivo lo calcula el servidor.
 */
const pad = (n: number) => String(n).padStart(2, '0');
const key = (y: number, m: number, d: number) => `${y}-${pad(m)}-${pad(d)}`;
const addDays = (date: Date, days: number) => new Date(date.getTime() + days * 86_400_000);

function easterSunday(year: number): Date {
  const a = year % 19;
  const b = Math.floor(year / 100);
  const c = year % 100;
  const d = Math.floor(b / 4);
  const e = b % 4;
  const f = Math.floor((b + 8) / 25);
  const g = Math.floor((b - f + 1) / 3);
  const h = (19 * a + b - d - g + 15) % 30;
  const i = Math.floor(c / 4);
  const k = c % 4;
  const l = (32 + 2 * e + 2 * i - h - k) % 7;
  const m = Math.floor((a + 11 * h + 22 * l) / 451);
  const month = Math.floor((h + l - 7 * m + 114) / 31);
  const day = ((h + l - 7 * m + 114) % 31) + 1;
  return new Date(Date.UTC(year, month - 1, day));
}

function nextMonday(date: Date): Date {
  const day = date.getUTCDay();
  const add = day === 1 ? 0 : (8 - day) % 7;
  return new Date(date.getTime() + add * 86_400_000);
}

function colombianHolidays(year: number): Set<string> {
  const fixed: [number, number][] = [
    [1, 1], [5, 1], [7, 20], [8, 7], [12, 8], [12, 25],
  ];
  const emiliani: [number, number][] = [
    [1, 6], [3, 19], [6, 29], [8, 15], [10, 12], [11, 1], [11, 11],
  ];
  const set = new Set<string>();
  for (const [m, d] of fixed) set.add(key(year, m, d));
  for (const [m, d] of emiliani) {
    const moved = nextMonday(new Date(Date.UTC(year, m - 1, d)));
    set.add(key(moved.getUTCFullYear(), moved.getUTCMonth() + 1, moved.getUTCDate()));
  }
  const easter = easterSunday(year);
  const easterBased: [Date, boolean][] = [
    [addDays(easter, -3), false],
    [addDays(easter, -2), false],
    [addDays(easter, 43), true],
    [addDays(easter, 64), true],
    [addDays(easter, 71), true],
  ];
  for (const [date, moves] of easterBased) {
    const dd = moves ? nextMonday(date) : date;
    set.add(key(dd.getUTCFullYear(), dd.getUTCMonth() + 1, dd.getUTCDate()));
  }
  return set;
}

const cache = new Map<number, Set<string>>();
function holidaysFor(year: number): Set<string> {
  let set = cache.get(year);
  if (!set) {
    set = colombianHolidays(year);
    cache.set(year, set);
  }
  return set;
}

function toUtcMidnight(date: Date): Date {
  return new Date(Date.UTC(date.getUTCFullYear(), date.getUTCMonth(), date.getUTCDate()));
}

export function countCalendarDays(start: Date, end: Date): number {
  const s = toUtcMidnight(start).getTime();
  const e = toUtcMidnight(end).getTime();
  if (e < s) return 0;
  return Math.floor((e - s) / 86_400_000) + 1;
}

export function countBusinessDays(start: Date, end: Date): number {
  let count = 0;
  let cursor = toUtcMidnight(start);
  const last = toUtcMidnight(end);
  while (cursor.getTime() <= last.getTime()) {
    const day = cursor.getUTCDay();
    const y = cursor.getUTCFullYear();
    const isWeekend = day === 0 || day === 6;
    const isHoliday = holidaysFor(y).has(key(y, cursor.getUTCMonth() + 1, cursor.getUTCDate()));
    if (!isWeekend && !isHoliday) count += 1;
    cursor = addDays(cursor, 1);
  }
  return count;
}

/** Previsualiza la duración de una ausencia según el modo de conteo. */
export function previewAbsenceDays(
  start: string,
  end: string,
  mode: 'BUSINESS' | 'CALENDAR',
): number {
  if (!start || !end) return 0;
  const s = new Date(`${start}T00:00:00Z`);
  const e = new Date(`${end}T00:00:00Z`);
  if (Number.isNaN(s.getTime()) || Number.isNaN(e.getTime()) || e < s) return 0;
  return mode === 'BUSINESS' ? countBusinessDays(s, e) : countCalendarDays(s, e);
}
